    ...
    with open('Data\Map__Crime_Incidents_-_from_1_Jan_2003.csv', 'rb') as f:
        ...
        for i, row in enumerate(reader):
            if i > 0:
                ...
                incident = Crime_Stat(incident_num=incident_num,category=category,description=description,map_category=map_category,day_of_week=day_of_week,date=date,time=time,district=district,x_cord=x_cord,y_cord=y_cord)
...
def load_recent_stats():
    ...
    for i, row in enumerate(reader):
        if i > 0:
            try:
                overlap = Crime_Stat.query.filter_by(incident_num=row[0]).one()
            except:
                    print type(row)
                    print row[0]
                    incident_num = row[0]
